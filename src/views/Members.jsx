import React, { useEffect, useState, useRef } from 'react';
import Layout from '../components/Layout/Layout';
import { useTranslation } from 'react-i18next';
import { useDispatch, useSelector } from 'react-redux';
import Modal from '../components/Common/Modal';
import { CREATE_MODE, INPUT_EMAIL, INPUT_NAME, MEMBER_ENTITY } from '../utils/consts';
import { createMember, getMembers } from '../store/teams';
import ViewHeader from '../components/Layout/ViewHeader';
import ViewContent from '../components/Layout/ViewContent';
import View from '../components/Layout/View';
import Table from './../components/Common/Table/Table';
import Delete from '../components/Common/Actions/Delete';
import EmailInput from './../components/Inputs/EmailInput';
import TextInput from './../components/Inputs/TextInput';
import Dropdown from '../components/Inputs/Dropdown';
import { getOrganizations } from '../store/organization';

const Members = () => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const createMemberRef = useRef();

  const { members, isLoading, error } = useSelector((state) => state.teams);
  const { currentOrganization, organizations } = useSelector((state) => state.organization);
  const { session } = useSelector((state) => state.user);

  const [formData, setFormData] = useState({
    email: '',
    name: '',
    organization: ''
  });
  const [errors, setErrors] = useState({});
  const [enableSubmit, setEnableSubmit] = useState(false);

  const onChange = (name, value) => {
    setFormData((prevState) => ({ ...prevState, [name]: value }));
  };

  const checkFormValid = () => {
    let valid = true;

    const dataLength = Object.keys(formData).length;
    const errorsLength = Object.keys(errors).length;

    if (dataLength === errorsLength) {
      for (let i = 0; i < Object.keys(errors).length; i++) {
        const element = Object.keys(errors)[i];

        if (errors[element].length > 0) {
          valid = false;
        }
      }
    } else {
      valid = false;
    }

    setEnableSubmit(valid);
  };

  const onSubmit = async (e) => {
    e.preventDefault();
    dispatch(createMember({ ...formData }));

    if (createMemberRef.current) {
      createMemberRef.current.checked = false;
    }

    setFormData({ email: '', name: '', organization: '' });
  };

  const handleOpenModal = () => {
    setFormData({ email: '', name: '', organization: '' });
    createMemberRef.current.checked = true;
  };

  useEffect(() => {
    checkFormValid();
  }, [formData]);

  useEffect(() => {
    if (currentOrganization !== '') {
      dispatch(getMembers(currentOrganization));
    }
    dispatch(getOrganizations(session.$id));
  }, [currentOrganization]);

  return (
    <Layout>
      <View>
        <ViewHeader title={t('menu.members')}>
          <button
            onClick={handleOpenModal}
            className="btn btn-sm btn-primary normal-case"
            disabled={error || isLoading}>
            {t(`actions.${CREATE_MODE}`) + ' ' + t(`entities.${MEMBER_ENTITY}`)}
          </button>
        </ViewHeader>
        <ViewContent>
          <Modal
            mode={CREATE_MODE}
            entity={t(`entities.${MEMBER_ENTITY}`)}
            composedKey={`${CREATE_MODE}__${MEMBER_ENTITY}`}
            modalRef={createMemberRef}>
            <form action="">
              <TextInput
                name={INPUT_NAME}
                required
                disabled={isLoading}
                onChange={onChange}
                value={formData.name}
                errors={errors.name}
                setErrors={setErrors}
              />
              <EmailInput
                name={INPUT_EMAIL}
                required
                disabled={isLoading}
                onChange={onChange}
                value={formData.email}
                errors={errors.email}
                setErrors={setErrors}
              />
              <Dropdown
                name="organization"
                required
                disabled={isLoading}
                onChange={onChange}
                value={formData.organization}
                errors={errors.organization}
                setErrors={setErrors}>
                {organizations.map((organization) => (
                  <option key={organization.$id} value={organization.$id}>
                    {organization.name}
                  </option>
                ))}
              </Dropdown>
              <div className="flex flex-col mt-6">
                <button
                  type="submit"
                  disabled={!enableSubmit || isLoading}
                  onClick={onSubmit}
                  className={`btn btn-primary ${isLoading ? 'loading' : ''}`}>
                  {t('actions.save')}
                </button>
              </div>
            </form>
          </Modal>
          <Table
            name={MEMBER_ENTITY}
            data={members}
            hidden={[
              '$id',
              '$updatedAt',
              '$permissions',
              '$collectionId',
              '$databaseId',
              'teamId',
              'avatar',
              'role',
              'userId',
              'organization'
            ]}
            controls={(row) => (
              <>
                <Delete
                  id={row.$id}
                  action={() => {
                    console.log('delete', row.$id);
                  }}
                  entity={MEMBER_ENTITY}
                />
              </>
            )}
          />
        </ViewContent>
      </View>
    </Layout>
  );
};

export default Members;
